...
n_lst = [50, 100, 500, 3000]
n = n_lst[4]
A_lst = 10. .^ range(-1, 2, length=30)
predict_matrix_friction = zeros(length(A_lst));
...
for j in 1:length(A_lst)
    ...
    predict_matrix_friction[j] = accuracy / repeats
    # RMSE_postmean[j] = RMSE / repeats
end

save_variable(predict_matrix_friction, "predict_matrix_friction_n4");
